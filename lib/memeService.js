...
async function listCachedTerms(options = {}) {
  ...
  return Object.values(db.queryCache)
    ...
    .sort(sortPublicTerms)
    .slice(0, limit);
}

async function listTrendingTerms(options = {}) {
  const config = getConfig();
  if (!config.useSupabase) {
    return [];
  }
  ...
  const { data, error } = await supabase
    .from("meme_trending_terms")
    .select("term, source, source_url, rank, heat, reason, category, tags, updated_at")
    .eq("status", "candidate")
  ...
  if (error) {
    throw new Error(`Supabase 热榜读取失败：${error.message}`);
  }
...
async function refreshTrendingTerms() {
  const config = getConfig();
  if (!config.useSupabase) {
    throw new Error("请先配置 Supabase，热榜候选需要写入云数据库。");
  }
  if (!config.apiKey) {
    throw new Error("请先配置 DEEPSEEK_API_KEY，热榜候选需要 LLM 筛选。");
  }

  const rawItems = await fetchWeiboHotSearch();
  const candidates = await selectTrendingMemes(rawItems.slice(0, 30), config);
  const now = new Date().toISOString();
  const supabase = getSupabaseClient(config);
  const rows = candidates.map((item) => {
    const source = rawItems.find((raw) => normalizeQuery(raw.term) === normalizeQuery(item.term));
    return {
      normalized: normalizeQuery(item.term),
      term: item.term,
      source: source?.source || "weibo",
      source_url: source?.url || null,
      rank: source?.rank || null,
      heat: source?.heat || null,
      reason: item.reason || "",
      category: item.category || "hot",
  ...
  const { error } = await supabase.from("meme_trending_terms").upsert(rows, { onConflict: "normalized" });
  if (error) {
    throw new Error(`Supabase 热榜写入失败：${error.message}`);
  }
...
JSON 结构：
{
  "term": "原词",
  "aliases": ["别名或相关叫法"],
  "tags": ["2 到 5 个短标签，如 评论区高频、情绪表达、轻微吐槽"],
  "one_liner": "一句话解释",
...
async function fetchWeiboHotSearch() {
  ...
  if (!response.ok) {
    throw new Error(`微博热搜读取失败：${response.status}`);
  }
  ...
  return items
    .map((item, index) => ({
      term: String(item.word || item.note || "").trim(),
      source: "weibo",
      url: item.word_scheme ? `https://s.weibo.com/weibo?q=${encodeURIComponent(item.word_scheme)}` : "https://s.weibo.com/top/summary",
      rank: index + 1,
      heat: item.num ? String(item.num) : ""
    }))
    .filter((item) => item.term && !item.term.includes("置顶"));
}

async function selectTrendingMemes(items, config) {
  if (!items.length) {
    return [];
  }
      ...
      messages: [
        {
          role: "system",
          content:
            "你是中文互联网热梗编辑。只输出严格 JSON。任务是从热搜标题里筛出适合放进梗词词典的候选。不要选纯新闻、人名事故、严肃灾难、广告、低俗攻击。"
        },
        {
          role: "user",
          content: `从下面热搜里选出最多 10 个更像网络梗、流行表达、评论区高频说法、抽象短语或可解释网络语境的候选。
输出 JSON：{"items":[{"term":"原词","reason":"为什么像梗或值得解释","category":"hot|classic|irony|emotion|abstract|abbr|entertainment|workplace","tags":["短标签1","短标签2"]}]}

热搜列表：
${items.map((item) => `${item.rank}. ${item.term}`).join("\n")}`
        }
      ]
    })
  });
  ...
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(payload.error?.message || "DeepSeek 热榜筛选失败。");
  }

  const content = payload.choices?.[0]?.message?.content;
  const parsed = JSON.parse(content || "{\"items\":[]}");
  return Array.isArray(parsed.items) ? parsed.items.filter((item) => item.term).slice(0, 10) : [];
}
...
function sortPublicTerms(a, b) {
  return Number(b.queryCount || 0) - Number(a.queryCount || 0) || new Date(b.lastQueriedAt || 0) - new Date(a.lastQueriedAt || 0);
}
...
function normalizeAiJson(value, originalTerm) {
  ...
  return {
    ...
    tags: normalizeTags(safeValue.tags, safeValue.category),
...
function normalizeTags(value, category) {
  const tags = arrayOfStrings(value)
    ...
    .slice(0, 5);
  const categoryTag = categoryLabels[category] || "网络表达";
  return [...new Set([categoryTag, ...tags])].slice(0, 5);
}
